...
import { getPelotones, putPelotones } from "../Api/Pelotones";
import { Link, useNavigate } from "react-router-dom";
import { putRangos } from "../Api/Rangos";

export function TablaPelotones() {
  const [pelotonesTabla, setPelotonesTabla] = useState([]);
  const [buscador, setBuscador] = useState("");
  const [mark, setMark] = useState("True");
  const navigate = useNavigate();

  // Obtiene los datos de la tabla Pelotones y los guarda en el estado de pelotonesTabla
  useEffect(() => {
    async function gettingPelotones() {
      const response = await getPelotones();
      setPelotonesTabla(response.data);
    }
    gettingPelotones();
  }, [mark]);
  ...
  // Función para filtrar y mostrar solo los pelotones activos
  const activos = () => {
    getPelotones().then((response) => {
      const data = response.data;
      setPelotonesTabla(
        data.filter((pelotones) => pelotones.activo === "True")
      );
    });
  };

  // Función para filtrar y mostrar solo los pelotones inactivos
  const inactivos = () => {
    getPelotones().then((response) => {
      const data = response.data;
      setPelotonesTabla(
        data.filter((pelotones) => pelotones.activo === "False")
      );
    });
  };
  ...
  // Muestra los datos por busqueda del nombre (pelotones)
  const results = !buscador
    ? pelotonesTabla
    : pelotonesTabla.filter((pelotones) =>
        pelotones.nombreP.toLowerCase().includes(buscador.toLowerCase())
      );
  ...
  return (
    <>
      ...
      <table class="w-full whitespace-nowrap">
        ...
        {pelotonesTabla[0] ? (
          <>
            {results.map((pelotonesTabla) => (
              <tr class="{index !== rangoTabla.length - 1 ? 'border-b border-black' : ''}">
                <td class="py-3 px-2 border border-black">
                  <div class="inline-flex space-x-3 items-center">
                    <span>{pelotonesTabla.nombreP}</span>
                  </div>
                </td>
                <td class="py-3 px-2 border border-black">
                  {pelotonesTabla.descripcionP}
                </td>
                <td class="py-3 px-2 border border-black">
                  {pelotonesTabla.activo === "True" ? (
                    <>Activo</>
                  ) : (
                    <>Inactivo</>
                  )}
                ...
                <td class="py-3 px-2 border border-black">
                  <div class="inline-flex items-center space-x-3">
                    <Link
                      ...
                      to={`/pelotones/${pelotonesTabla.idPeloton}`}
                    >
                      <svg
                    ...
                    <button
                      className="hover:text-white"
                      value={pelotonesTabla.idPeloton}
                      onClick={(e) => {
                        const data = {
                          idPeloton: e.currentTarget.value,
                          activo: "True",
                        };
                        putPelotones(data);
